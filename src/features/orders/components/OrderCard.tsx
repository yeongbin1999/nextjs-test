  ...
  return (
    <Card className="p-6 bg-white/80 border-amber-100 shadow-lg rounded-2xl h-full flex flex-col justify-between">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-3">
        <div className="text-brown-900 font-semibold">주문번호: {order.orderId}</div>
        <div className="text-brown-700 text-sm">{formatDate(order.orderDate)}</div>
        <span
      ...
      <div className="text-brown-800 text-base mb-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm text-brown-600">주문자: {order.username || '알 수 없음'}</span>
        </div>
      </div>
