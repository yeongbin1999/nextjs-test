...
export async function addToCart(request: AddCartItemRequest): Promise<CartItem[]> {
  const userId = useAuthStore.getState().user?.id;
  if (!userId) throw new Error('로그인이 필요합니다');
  await apiClient.api.addItem({ userId }, { productId: request.productId, quantity: request.quantity });
  return fetchCart();
}

// 장바구니 상품 수량 업데이트
export async function updateCartItem({ itemId, quantity }: { itemId: number; quantity: number }): Promise<CartItem[]> {
  const userId = useAuthStore.getState().user?.id;
  if (!userId) throw new Error('로그인이 필요합니다');
